...
async function clearData() {
  try {
    const operation = await sendDataTo({ url: `${localhost}/system/data/clear`, method: 'POST', useHeaders: true }, undefined);

    if (!operation.success) {
      return {
        success: false,
        message: 'Ошибка (clear-data): Incorrect server response'
      };
    }
    ...
    if (error instanceof TypeError) {
      return {
        success: false,
        message: `Ошибка (clear-data): Server not responding`
      };
    } else {
      return {
        success: false,
        message: `Ошибка (clear-data): ${error}`
      };
    }
  }
}
      ...
      const initButtonsInMainContainer = async () => {
        ...
        startBotsProcessBtn.addEventListener('click', async () => {
          try {
            ...
            const useProxyChecker = (document.getElementById('use-proxy-checker')).checked;
        ...
        stopBotsProcessBtn.addEventListener('click', async () => {
          try {
            ...
            await clearData().then(({ success, message }) => success ? log(message, 'log-system') : log(message, 'log-error'));
          } catch (error) {
        ...
        try {
          ...
          startSpammingBtn.addEventListener('click', async () => {
            ...
            try {
              ...
              if (message === '') {
                log('Поле "Сообщение" пустое, действие невозможно', 'log-warning'); return;
              }
  ...
  async initializeCheckboxFunctions() {
    try {
      const initCheckboxesInSettingsContainer = async () => {
        try {
          const proxyChbx = document.getElementById('use-proxy');
          const proxyCheckerChbx = document.getElementById('use-proxy-checker-container');
...
async function checkUpdate() {
  try {
    ...
    const copyDownloadLink = document.getElementById('copy-download-link');
