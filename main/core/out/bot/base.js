    ...
    try {
        ...
        for (let i = 0; i < options.quantity; i++) {
            ...
            for (let attempts = 0; attempts < 6; attempts++) {
                ...
                if (!Object.keys(flow).includes(username))
                    break;
                username = null;
            }
            if (username) {
...
export async function manipulateBot(action, options) {
    switch (action) {
        case 'recreate':
...
export function updateBotTask(username, task, status, load) {
        ...
        bot.on('end', async (reason) => {
            try {
                if (reason === '@salarixi:disconnect')
                    return;
                if (profile.rejoinProcess === 'active')
                    return;
...
async function authorizeBot(username, bot, method) {
    try {
        const options = settings.get(username);
        if (options.useExtendedLogs)
            msg('process:botting', {
...
async function recreateBot(username) {
    try {
        const options = settings.get(username);
        if (options.useExtendedLogs)
            msg('process:botting', {
...
async function resetBot(username) {
    const options = settings.get(username);
    const profile = flow[username]?.profile;
    if (!profile)
        return;
