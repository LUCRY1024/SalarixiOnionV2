...
export class FlightPlugin {
    ...
    async flight(username, bot, options) {
        try {
            if (options.state === 'start') {
                if (flow[username]?.profile?.tasks.flight.status)
                    return;
                updateBotTask(username, 'flight', true, 3.8);
                switch (options.type) {
                    case 'default':
                        await this.defaultFlight(username, bot, options);
                        break;
                    case 'jump':
                        await this.jumpFlight(username, bot, options);
                        break;
                    case 'glitch':
                        await this.glitchFlight(username, bot, options);
                        break;
                }
            }
    ...
    async defaultFlight(username, bot, options) {
        if (!bot)
            return;
    ...
    async jumpFlight(username, bot, options) {
        if (!bot)
            return;
        let isFirst = true;
        while (flow[username]?.profile?.tasks.flight.status) {
    ...
    async glitchFlight(username, bot, options) {
        if (!bot)
            return;
        let isFirst = true;
        while (flow[username]?.profile?.tasks.flight.status) {
