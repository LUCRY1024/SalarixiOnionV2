...
export class ChatPlugin {
    async flow(username, bot, options) {
        switch (options.type) {
            case 'message':
                await this.message(username, bot, options);
                break;
            case 'spamming':
                await this.spamming(username, bot, options);
                break;
        }
    }
    createMagicText(text) {
        let magicText = '';
    ...
    async message(username, bot, options) {
        try {
            if (!bot)
                return;
    ...
    async spamming(username, bot, options) {
        if (!bot)
            return;
