...
export class ImitationPlugin {
    ...
    async flow(username, bot, options) {
        switch (options.type) {
            case 'hybrid':
                await this.hybridImitation(username, bot, options);
                break;
            case 'walking':
                await this.walkingImitation(username, bot, options);
                break;
        }
    }
    ...
    async looking(username, bot, useSmoothness, useLongDelays) {
        if (!bot)
            return;
        if (flow[username]?.profile?.tasks.looking.status)
            return;
        try {
            ...
            while (flow[username]?.profile?.tasks.looking.status) {
                if (Math.random() >= 0.8) {
                    ...
                    if (entity && entity.type === 'player') {
                        bot.lookAt(entity.position.offset(chooseRandomElementFromArray([Math.random() / 3, -(Math.random() / 3)]), entity.height, chooseRandomElementFromArray([Math.random() / 3, -(Math.random() / 3)])), !useSmoothness);
                    }
                }
                else {
                    const vector = new Vec3(bot.entity.position.x + generateNumber('float', -1, 1), bot.entity.position.y + generateNumber('float', -0.1, 0.1), bot.entity.position.z + generateNumber('float', -1, 1));
    ...
    async waving(username, bot, useLongDelays) {
        if (!bot)
            return;
        if (flow[username]?.profile?.tasks.waving.status)
            return;
        try {
    ...
    async hybridImitation(username, bot, options) {
        if (options.state === 'start') {
            if (!bot)
                return;
            if (flow[username]?.profile?.tasks.hybridImitation.status)
                return;
            if (flow[username]?.profile?.tasks.jumping.status || flow[username]?.profile?.tasks.shifting.status || flow[username]?.profile?.tasks.moveForward.status || flow[username]?.profile?.tasks.moveBack.status || flow[username]?.profile?.tasks.moveLeft.status || flow[username]?.profile?.tasks.moveRight.status)
                return;
            if (!options)
                return;
            try {
                ...
                while (flow[username]?.profile?.tasks.hybridImitation.status) {
                    if (isFirst) {
                        ...
                        if (options.useWaving)
                            this.waving(username, bot, options.useLongDelays);
                        isFirst = false;
                    }
                    let chain = '';
                    let attempts = 0;
                    ...
                    for (const operation of operations) {
                        ...
                        for (const action of actions) {
                            if (!flow[username]?.profile?.tasks.hybridImitation.status || !flow[username]?.profile?.tasks.jumping.status || !flow[username]?.profile?.tasks.shifting.status || !flow[username]?.profile?.tasks.moveForward.status || !flow[username]?.profile?.tasks.moveBack.status || !flow[username]?.profile?.tasks.moveLeft.status || !flow[username]?.profile?.tasks.moveRight.status)
                                break;
    ...
    async walkingImitation(username, bot, options) {
        if (options.state === 'start') {
            if (!bot)
                return;
            if (flow[username]?.profile?.tasks.walkingImitation.status)
                return;
            if (flow[username]?.profile?.tasks.moveForward.status || flow[username]?.profile?.tasks.moveBack.status || flow[username]?.profile?.tasks.moveLeft.status || flow[username]?.profile?.tasks.moveRight.status || flow[username]?.profile?.tasks.looking.status)
                return;
            if (!options)
                return;
            try {
                ...
                while (flow[username]?.profile?.tasks.walkingImitation.status) {
                    ...
                    if (!flow[username]?.profile?.tasks.walkingImitation.status || !flow[username]?.profile?.tasks.moveForward.status || !flow[username]?.profile?.tasks.moveBack.status || !flow[username]?.profile?.tasks.moveLeft.status || !flow[username]?.profile?.tasks.moveRight.status)
                        break;
