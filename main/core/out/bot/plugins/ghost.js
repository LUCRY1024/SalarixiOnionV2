...
export class GhostPlugin {
    ...
    packetBuffering(bot, packetName, packetData) {
    ...
    async clearPacketBuffer(bot) {
        if (!bot)
            return;
        if (this.packetBuffer[bot.username].size === 0)
            return;
    ...
    async temperatePacketIgnore(username, bot, options) {
        try {
            if (!bot)
                return;
    ...
    async normalPacketIgnore(username, bot, options) {
        try {
            if (!bot)
                return;
            if (!this.originalWrite[bot.username]) {
    ...
    async aggressivePacketIgnore(username, bot, options) {
        try {
            if (!bot)
                return;
            if (!this.originalWrite[bot.username]) {
            ...
            bot._client.write = async (packetName, packetData) => {
                ...
                if (flow[username]?.profile?.tasks.ghost.status && this.activePacketIgnore[bot.username]) {
                    ...
                    if (options.useBuffering && Math.random() > 0.2)
                        this.packetBuffering(bot, packetName, packetData);
                    if (Math.random() > 0.95) {
                        this.clearPacketBuffer(bot);
                    }
    ...
    async ghost(username, bot, options) {
