...
export class AttackPlugin {
    ...
    async aiming(username, bot, type) {
    ...
    dodging(username, bot) {
        const interval = setInterval(async () => {
            ...
            if (!bot)
                return;
            const directions1 = ['forward', 'back'];
            const directions2 = ['left', 'right'];
            const direction1 = chooseRandomElementFromArray(directions1);
            const direction2 = chooseRandomElementFromArray(directions2);
    ...
    async attack(username, bot, options) {
        if (options.state === 'start') {
            ...
            if (options.useDodging)
                this.dodging(username, bot);
            ...
            while (flow[username]?.profile?.tasks.attack.status) {
                ...
                if (options.useImprovedStrikes) {
                    const action = chooseRandomElementFromArray(['jump', 'sneak']);
