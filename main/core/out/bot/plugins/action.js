...
export class ActionPlugin {
    ...
    async flow(username, bot, options) {
        switch (options.type) {
            case 'jumping':
                await this.jumping(username, bot, options);
                break;
            case 'shifting':
                await this.shifting(username, bot, options);
                break;
            case 'waving':
                await this.waving(username, bot, options);
                break;
            case 'spinning':
                await this.spinning(username, bot, options);
                break;
        }
    }
    async jumping(username, bot, options) {
    ...
    async shifting(username, bot, options) {
        if (options.state === 'start') {
            if (!bot)
                return;
    ...
    async waving(username, bot, options) {
        if (options.state === 'start') {
            if (!bot)
                return;
    ...
    async spinning(username, bot, options) {
        if (options.state === 'start') {
            if (!bot)
                return;
            if (flow[username]?.profile?.tasks.spinning.status)
                return;
            try {
