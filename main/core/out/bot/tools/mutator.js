...
export function mutateText(options) {
    let text = String(options.text);
    text = text.replace(/\[(.+)\]/g, (_, value) => chooseRandomElementFromArray(value.split(';')));
    text = text.replace(/#([nlms])\((\d+)-(\d+)\)/g, (match, type, min, max) => {
        let result = match;
        switch (type) {
            case 'n':
                result = generateString('numeric', generateNumber('int', Number(min), Number(max)));
                break;
            case 'l':
                result = generateString('letter', generateNumber('int', Number(min), Number(max)));
                break;
            case 'm':
                result = generateString('multi', generateNumber('int', Number(min), Number(max)));
                break;
            case 's':
                result = generateString('special', generateNumber('int', Number(min), Number(max)));
                break;
            default:
                break;
        }
