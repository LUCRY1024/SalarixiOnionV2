...
async function startProxyCollect(options) {
    try {
        let cmd = '';
    ...
    try {
        ...
        switch (cmd) {
            ...
            case 'open-file':
                ...
                break;
            case 'open-url':
                result = await shell.openExternal(options.url);
                break;
            case 'load-config':
                result = await configurator.load();
                break;
            case 'save-config':
                result = await configurator.save(options.config);
                break;
            case 'scrape-proxies':
                result = await startProxyCollect(options);
                break;
        }
